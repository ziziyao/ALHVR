...
def getPrototype_3D(fts, mask, region=False):
    """
    ...
    if torch.is_tensor(region):
        features = [[getFeatures_3D(fts[B,...], mask[B,C,...], region[B,...]) for B in range(batch_size)] for C in range(num_classes)]
    else:
        features = [[getFeatures_3D(fts[B,...], mask[B,C,...]) for B in range(batch_size)] for C in range(num_classes)]
    prototypes = [torch.unsqueeze(torch.sum(torch.cat(class_fts),dim=0),0) / batch_size  for class_fts in features]
    return prototypes

def getFeatures_3D(fts, mask, region=False):
    """
...
def getPrototype_2D(fts, mask, region=False):
    ...
    batch_size = mask.shape[0]
    if torch.is_tensor(region):
        features = [[getFeatures_2D(fts[B,...], mask[B,C,...], region[B,...]) for B in range(batch_size)] for C in range(num_classes)]
    else:
        features = [[getFeatures_2D(fts[B,...], mask[B,C,...]) for B in range(batch_size)] for C in range(num_classes)]
    prototypes = [torch.unsqueeze(torch.sum(torch.cat(class_fts),dim=0),0) / batch_size  for class_fts in features]
    return prototypes
...
def calDist_3D(fts, prototype, scaler=1.):
    """
